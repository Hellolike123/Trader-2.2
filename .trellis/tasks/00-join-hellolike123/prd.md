...
`hellolike123` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/hellolike123/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/hellolike123/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee hellolike123` to
...
python3 ./.trellis/scripts/task.py archive 00-join-hellolike123
```
