...
```
分析报告 — {name}（{code}）｜短中线
现价（含 MA20/MA250）+ 顶栏：环境（宽基/主交易板块/相对强弱）→ 概念标签 → 量能（量比/调整/位置/ATR14）
  （不写正常/偏弱/跑赢；环境档跟板块指数算，只进内部逻辑）
🧭 中线 → 阶段 + 定论 + 周线威科夫/缠论 + 位置 + 关键价（生命线/回踩/压力/目标）
⚡ 短线 → 结构 + 日线缠/阶段对照/事件/动能（综合总词）+ 资金（买卖盘）+ 共振 + 新开(C1) + 动作/原因/破位 + 关键价（止损/买点/🌟/卖点）
...
1. **HMM 与 MA 大势的“前瞻融合”**：
   ...
   - **人读面板**：`short_midline` 顶栏展示宽基+主交易板块指数涨跌与相对强弱、概念标签、量能/位置/ATR，**不写** `level`（正常/偏弱）；`level` 仍供 fusion / 纪律内部使用。
...
> 4. **禁用 `|...|` 表格**：一律禁止使用 Markdown 语法渲染的表格。如果多列数据需要并列显示，请用中文全角竖线 `｜` 或空格在单行内直接隔开（如 `现价 59.33（+2.70%）` 或 `环境：科创 +2.99% ｜ 电气 -3.44% ｜ 弱于板块 -0.80%`）。
> 5. **禁用 `>` 块引用**：一律禁止使用块引用。
...
现价 58.70（-3.77%）｜MA20 56.72｜MA250 49.66
  环境：科创 +2.99% ｜ 电气 -3.44% ｜ 弱于板块 -0.33%
  概念：特高压 ｜ 电网
  量能：量比0.9 平量 ｜ 换手0.7% ｜ 位置 转弱 ｜ ATR14 1.85

🧭 中线｜🔴 防守
  威科夫：吸筹早期 · 箱体未成形 · 仅对照
  缠论：拉升趋势中 · 看涨 · 线段偏少
  筹码：撑 56.50 · 压 59.00 · 主峰在下方
...
⚡ 短线｜🔴 不新开
  ...
  动作：不新开 · 不追现价 · 仓 0%
  原因：亏3.2/赚1.6，不划算
  破位看：跌破MA20(56.72)站不回；或破止损 55.51
