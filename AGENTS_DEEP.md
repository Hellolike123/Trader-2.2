...
- **四阶段定位模型**：蓄势/蓄势偏强/蓄势偏弱/主升/派发/衰退 × 走强/修复/震荡/转弱，贯穿仓位轮动、盘后复盘（选股池现为**软门槛 + 分道**，见 §3.1）
- **分析引擎并行化**：多票分析场景下缠论/威科夫/筹码等模块并行执行，显著提升选股池批量分析速度
- **入池（历史「三关」）**：旧「阶段+基本面+技术面硬拒」已废止；现行软入池后按 `lane` 分道（可盯/等齐/先别碰…），见 BUSINESS §4.2
...
```
分析报告 — {name}（{code}）｜短中线
现价（含 MA20/MA250）+ meta 纯 D：综合动能｜板块指数±%｜行业短名±%｜个股±%
  （不写正常/偏弱/跑赢；环境档跟板块指数算，只进内部逻辑）
🧭 中线 → 阶段 + 定论 + 周线威科夫/缠论 + 位置 + 关键价（生命线/回踩/压力/目标）
⚡ 短线 → 结构 + 日线缠/阶段对照/事件/动能 + 共振 + 新开(C1) + 动作/原因/破位 + 关键价（止损/买点/🌟/卖点）
...
ThreadPoolExecutor 并行执行策略（run_analysis.py）
  ...
  ├── _fetch_market_env()       ← 板块对照指数环境（resolve_board_index）
  ↓
...
1. **HMM 与 MA 大势的“前瞻融合”**：
   - 入口在 `market_env.py`。单票报告经 `resolve_board_index(sec)` 选定对照指数（科创50/创业板指/上证/深成；否则回退 `INDEX_CODE` 中证1000），再 `assess(index_code=…)` / `get_env_for_skill(..., index_code=…)`。文件缓存按指数分桶（`index_000688_SH` 等）。
   - `assess()` 用腾讯实时报价 + `fetch_qfq_daily(days=90)` 拉该指数前复权日线（注意：不能用 `fetch_kline(scale="240")`，后者是 Sina 分钟线）。
   - 缓存合并**按日期去重**：非今日缓存 bars + 当日实时 bar，防止 bars 无限膨胀。
   - 收益率序列喂入 `hmm_regime.detect_regime()`；均线 `level` 在 HMM `confidence >= 0.75` 时可前瞻修正（如 bear+正常→偏弱）。
   - **人读面板**：`short_midline` meta 纯 D 只展示板块指数/行业/个股涨跌，**不写** `level`（正常/偏弱）；`level` 仍供 fusion / 纪律内部使用。
...
> 4. **禁用 `|...|` 表格**：一律禁止使用 Markdown 语法渲染的表格。如果多列数据需要并列显示，请用中文全角竖线 `｜` 或空格在单行内直接隔开（如 `现价 59.33（+2.70%）` 或 `综合动能 震荡 ｜ 科创 +2.99% ｜ 电气 -3.44% ｜ 个股 +0.84%`）。
> 5. **禁用 `>` 块引用**：一律禁止使用块引用。
