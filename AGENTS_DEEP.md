...
```
分析报告 — {name}（{code}）｜短中线
现价（含 MA20/MA250）+ 顶栏：环境（宽基/主交易板块/相对强弱）→ 概念标签 → 量能（量比/调整/动能/ATR14）
...
1. **HMM 与 MA 大势的“前瞻融合”**：
   ...
   - **人读面板**：`short_midline` 顶栏展示宽基+主交易板块指数涨跌与相对强弱、概念标签、量能/动能/ATR，**不写** `level`（正常/偏弱）；`level` 仍供 fusion / 纪律内部使用。
...
现价 58.70（-3.77%）｜MA20 56.72｜MA250 49.66
  ...
  概念：特高压 ｜ 电网
  量能：量比0.9 平量 ｜ 换手0.7% ｜ 动能 转弱 ｜ ATR14 1.85
