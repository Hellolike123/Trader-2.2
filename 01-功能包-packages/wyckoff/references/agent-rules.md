...
顶栏（现行）：`环境：{宽基} ±% ｜ {主交易板块?} ±% ｜ 强于/弱于/持平板块` → `概念：标签…`（身份，禁止概念假指数）→ `量能：量比/换手/调整/位置/ATR14`。禁止再写「大盘 正常/偏弱」或单独「行业：…｜跑赢…」行；位置不进环境行。短线 `动能：` 先给 `偏强/中性/偏弱` 总词（无 direction 或 reason 含“数据不足”不硬加）；`资金：` 有依据才前置 `买盘占优/卖盘占优`。
