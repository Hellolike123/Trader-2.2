...
|--------|----------|
| 中短线面板文案 | 上面三步 |
| 新开 / 出手收紧 | `chan_discipline` / `mistery_gate` / `decision_view`（C1 用 `format_entry_line_c1`） |
| Fusion 席位（生产） | `analysis/fusion_card_signals.py`（cards 路径） |
| 微信红线本身 | 只改本文件，再 sync 三 Skill 的 `references/agent-rules.md` |

生产唯一渲染：短中线（`SHORT_MIDLINE_REPORT=false` 已忽略）。Fusion 默认 `cards`；classic 仅对照（deprecated）。
