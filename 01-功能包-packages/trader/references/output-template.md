...
- **标题**：`分析报告 — {名}（{码}）｜短中线`
- **meta**：`综合动能 … ｜ {上证/深成/创业板/科创} ±x% ｜ {行业短名} ±x% ｜ 个股 ±x%`（纯 D：不写正常/偏弱/跑赢；环境档跟板块指数算但 meta 不露）；MA 行须含 MA20 与 MA250；量价行：`量比… ｜ 换手… ｜ 调整N天/创新高 ｜ ATR14 x.xx（前/后/未复权）`（ATR **并入同行**，禁止再单独成行）；中线阶段细读在 🧭 威科夫行（无独立「阶段：」）
- **🧭 中线**
