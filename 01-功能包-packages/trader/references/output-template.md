...
现价 48.50（+2.00%）｜MA20 46.00｜MA250 40.00
  环境：深成 +0.50% ｜ 电气 -1.20% ｜ 强于板块 +3.20% ｜ 动能 震荡
  量能：量比1.2 平量 ｜ 换手4.0%
...
- **标题**：`分析报告 — {名}（{码}）｜短中线`
- **顶栏 A**：①`现价…｜MA20｜MA250` ②`环境：{指数} ±% ｜ {行业?} ±% ｜ 强于/弱于/持平板块 … ｜ 动能 …`（**无个股绝对%**，现价行已有；相对板块用超额涨跌；不写正常/偏弱/跑赢）③`量能：量比… 平/放/缩 ｜ 换手? ｜ 调整N天/创新高`（**默认无 ATR、无资金面/两融**；|两融净买|≥5000万才点名 `两融：`）；阶段主展示在 🧭
- **🧭 中线**
