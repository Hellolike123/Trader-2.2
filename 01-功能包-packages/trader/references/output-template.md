...
- **🧭 中线**
  ...
  - 关键价（中线）：**价格升序阶梯**（价在前）；生命线/黄金/MA20/MA250/压力/目标；**无 🌟**；最多约 6 档；偏空时与生命线重叠的大回踩区可省略；>+80% 远压默认不进阶梯，>+50% 标「远」；关闭态/偏空禁「低吸」动词
- **⚡ 短线（A 版读序 · 窄屏扫读）**
  - 顺序：`缠论` → `买点?` → `威科夫`（含事件）→ `动能` → `资金` →（空行）→ `共振/新开` → `动作` → `原因?` → `破位看` → 关键价
  - `缠论：` ← 日线缠论扳机（与中线点名同构；主信号最多三段）
  - 关键价（短线）：**价格升序阶梯**；关闭态=止损+MA5?+现价+站稳线（confirm），**不铺**计划买区/VWAP/止盈旅游图；放行态才出低吸区/止盈区
