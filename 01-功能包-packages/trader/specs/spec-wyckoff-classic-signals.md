...
**触发条件**：
1. 最近 5 根 K 线窗口满足：
   - ≥4/5 阳线（`close > open`；实现已放宽，非必须 5 连阳）
   - `close[4] >= open[0]`（总体抬高）
   - 平均成交量 > 前 10 日均量 * 1.2
...
**触发条件**（时序：SOS → 回调 → LPS，不可颠倒）：
1. 从近到远定位最近一次有效 SOS（SOS 阳线标准与 `_detect_sos` 一致：≥4/5）
2. 仅评估该 SOS **之后**到当前的完整回调（2–10 根）；回调失败不回退更早伪 SOS
3. 回调：末收 ≤ 起始收盘 ×1.01（下行或横盘近似）
4. 回调最低价 ≥ SOS 前低 ×0.99（不破前低，1% 容差）
5. 回调末端成交量 < 基线均量 ×0.7（缩量确认）
...
归一化: WYCKOFF_SCORE_MAX_ABS 从 80 → 95 (取 max(85, 55) * 1.1 = 93.5, 取整为 95 留余量)
```
