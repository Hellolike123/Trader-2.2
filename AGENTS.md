...
- **Fusion 生产路径**：`FUSION_FROM_CARDS` 缺省 = `cards`；失败 → `cards_failed` 中性占位（**禁止**静默回退 classic）；`classic` / `compare` 仅对照（classic deprecated）。详见 `BUSINESS.md` §2.7。**A1** 已落地（merge 在 stage_pack 后、短中线前）；**A2 仍延期**（勿把 merge 挪到 decision_view 之后）。
...
| Fusion 生产路径 | `fusion_core.py` + `analysis/cards.py` + `fusion_card_signals.py`（失败=`cards_failed`） | 加厚 classic 当主路径；生产静默回退 classic |
| Fusion 置信度 | `fusion_confidence.py`（中性；classic_mappers re-export） | analysis 经 classic_mappers 再取置信 |
