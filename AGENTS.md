...
- **T0 结构参考卡 v2.1（2026-07-29）**：人读结构仪表盘（法源 `docs/t0-strategy-v2.md`）。骨架：标题 → 位置叙事结论（人决策）→ `📌 结构`（位置/量能/空间/关注价）→ 有仓才 `📋 若做正T` → `🔗 参考`（评分不进结论主语）→ 看法失效 → 资金 → 持仓纪律。禁止「可执行/可低吸/三重共振买」。VWAP 只用今日数据；日线 120 / 5m 800。机械 5m 信号回测已收工，不进自动执行。
