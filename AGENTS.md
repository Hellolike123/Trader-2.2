...
- **开仓清单 C1**：五项布尔（中线趋势｜回踩到位｜买点信号｜融合置信｜筹码资金稳）；`融合置信`=融合置信够（非三席方向一致）。一行 `新开：否（缺：…）` 或 `新开：可试探（清单全绿）`；不全绿不写试探买。
- **买点盖生命周期**：收盘破盖=失败取消买点；盘中刺穿收盘收回=不判死；失败后站回须重走「站上→回踩」。规格 `docs/designs/buy-point-lid-lifecycle.md`（实现未全落地）。
- **四阶段定位模型**：蓄势/蓄势偏强/蓄势偏弱/主升/派发/衰退 × 走强/修复/震荡/转弱，贯穿选股池入池三关、仓位轮动、盘后复盘全链路；双轨报告中「阶段」只出现在 🧭 中线 `阶段：`，不进中线「看法」。
...
- **T0 执行卡风格（2026-07-20）**：T0 输出从 4 段式改为执行卡风格——标题 → 结论（一句话）→ 执行价（低吸/高抛/止盈）→ 信号（三重共振）→ 失效条件 → 资金 → 降本模式。三重共振第一席位从缠论替换为 Al Brooks 价格行为（Always-In + 信号棒 + follow-through + H/L 回调计数），VWAP 只用今日数据（避免跨日高价拉高），止盈按方向拆分（低吸止盈高于现价 / 高抛止盈低于现价），信号棒价格远离现价 >20% 自动过滤。数据长度：日线 120 根（原 30）、5m 800 根。
...
- **斐波那契黄金挂单位 (Golden Bid)**：`structure_core.py` 自动从缠论笔中计算 38.2%/50%/61.8% 黄金分割回调价，并与当前低吸价格区间求交集；进入 `key_prices`/买点区逻辑，由双轨报告的「关键价（短线）」展示，不再挂在旧 `📍 决策` 区块。
...
- **双轨亮点/风险**：`render_short_midline` 按中线看法、出手、止损、中线压力/生命线生成 `✅ 亮点` / `⚠️ 风险`（禁止「阶段…中线故事」挂羊头）。
...
单票报告的主契约是**中短线双轨 + 纪律出手**（见 output-template / `render_short_midline`）。
内部仍保留双层状态字段供兼容：
- `base_status` 结构位置层；`theory_status` 理论结论层；`state_label` 兼容摘要
- `major_stage` 进 🧭 `阶段：`；方向仍以 `fusion.weighted_score` 为准
- 旧的 `scene` / 旧 `🎯`+`📍 决策` 布局不得再当主输出

接入本仓库的 AI 协同系统优先阅读本页，随后查阅 `output-template.md`、`output-style-guide.md` 和 「SKILL.md」 以统一输出契约。优先跑脚本拿 markdown，禁止凭记忆拼旧面板。
