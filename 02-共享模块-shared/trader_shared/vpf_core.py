    ...
    *,
    ...
    if has_fund_series:
        ...
        if con_out >= 3:
            ...
            if abs(cum5_f) >= 100:
                fund_reason += f"（5日合计{abs(cum5_f):.0f}万）"
        elif con_out >= 2:
            fund_dir = -1
            fund_conf = 0.55
            fund_reason = f"连{con_out}日净出"
            ...
            if avg_daily_turnover_wan and avg_daily_turnover_wan > 0:
                ...
                fund_reason = f"5日净出{abs(cum5_f):.0f}万（占比{_strength:.2%}）"
            else:
                fund_conf = 0.4
                fund_reason = f"5日净出{abs(cum5_f):.0f}万"
        elif cum5_f >= 500:
            fund_dir = 1
            if avg_daily_turnover_wan and avg_daily_turnover_wan > 0:
                ...
                if _strength >= 0.01:
                    ...
                    fund_conf = 0.15
                fund_reason = f"5日净进{abs(cum5_f):.0f}万（占比{_strength:.2%}）"
            else:
                fund_conf = 0.35
                fund_reason = f"5日净进{abs(cum5_f):.0f}万"
        else:
            fund_dir = 0
            fund_conf = 0.25
            fund_reason = "资金中性"
