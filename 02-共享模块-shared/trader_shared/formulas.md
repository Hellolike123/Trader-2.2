...
- **§2.1a 衔接延伸**：若成下一笔前，扫描把起点推到更极端同向（更深底/更高顶），
  且上一笔终点同型，则先把上一笔 `end` 延到新起点再成新笔——保证价格连续，
  避免「下笔 end=57、次笔 up start=37」断裂假笔（南网日线）。初选「第一个合格反向」不变。
- **§2.1b 延伸护栏**：延伸（衔接或无反向时的同向延伸）不得跨过破坏笔起点极值的反向分型——
  向下笔区间内若有 `high > start_price` 的顶、向上笔区间内若有 `low < start_price` 的底，
  则禁止延伸（华工：防 181→90 吞掉中间 187 冲高）。
- **§2.1c 破极值短笔**：反向分型 K 线距不足时，若它破坏上一笔起点极值
  （下笔后更高顶 / 上笔后更低底），则破例成笔。起点可已推到更极端同向，
  但须仍在上一笔终点一侧延续（更深底 / 更高顶）。与 §2.1b 配套：禁延伸时
  不得留下价格断层（中航光电：40.74→38.62）、也不得丢掉中间结构。
...
**§3.4 起终点**：与 `direction` 一致——向上线段 `start=段内low → end=段内high`，
向下线段 `start=段内high → end=段内low`（`_segment_range`）。
禁止用「首笔 min/max + 末笔 max/min」导致未完成段出现向上 end<start / 向下 end>start。
若段首为与段方向相反的共用转折笔，排除其远端极值（下行段不吃上笔旧低、上行段不吃下笔旧高）。

**§3.5 未完成段纠偏**：收尾时若尚无已完成段，且末笔已突破段首极值并与
`current_direction` 拧句（如下行段末笔创新高），则按净走势翻转展示方向
（华工周线：避免整段 `down 187→22`）。

**§3.5b 形成中下行@高点**：已有完成上段后，未完成下行若末笔已回到片段高点——
创出上一上段新高则并回该上段（中际旭创周线）；未破前高则不输出该未完成下行
（中科曙光周线）。

**§3.6 第二类线段破坏（Bug R）**：与 §3.1 三分型终结 **OR 并存**，特征序列元素
突破段起点极值即破坏，不要求整根脱离：
- 向上段：特征序列（向下笔）的 `low` 跌破段起点 `low` → 段破坏
