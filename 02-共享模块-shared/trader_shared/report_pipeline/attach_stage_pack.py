...
def attach_stage_position_pack(
    ...
    *,
    ...
    """Thin：持仓/仓位包；字段从 StageContext 取。返回 (report, cost, has_position, suggested)。"""
    return _attach_stage_position_pack_impl(
        report,
        cost_price=float(ctx.get("cost_price") or 0),
        ...
        stage_result=ctx.get("stage_result"),
        atr14_val=ctx.get("atr14_val"),
        bars=ctx.get("bars") or [],
        wyck_result=ctx.get("wyck_result"),
        support=ctx.get("support"),
        confirm=ctx.get("confirm"),
        expma10_val=ctx.get("expma10_val"),
        expma20_val=ctx.get("expma20_val"),
        chip_migration=ctx.get("chip_migration"),
        levels=ctx.get("levels") if isinstance(ctx.get("levels"), dict) else {},
        bars_date=ctx.get("bars_date"),
...
def _attach_stage_position_pack_impl(
    report: dict[str, Any],
    *,
    cost_price: float,
    current: float,
    ...
    if suggested == 0:
        if hint_action in _reduce_set or any(k in hint_action for k in ("减仓", "空仓", "止损")):
