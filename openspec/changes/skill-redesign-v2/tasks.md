...
- [ ] 1.5 验证：执行 `python3 final_report.py --target 688248 --output json` 输出有效 JSON，包含全部 63+ 个字段
...
- [ ] 2.4 验证：执行 t0 script --target 688248 --once --output json 输出有效 JSON
...
- [ ] 3.3 验证：执行 review script --target 688248 --output json 输出有效 JSON
...
- [ ] 7.1 验证 trader --output json 输出的 JSON 可被 Python json.loads() 解析
- [ ] 7.2 验证 t0 --once --output json 输出的 JSON 可被解析
- [ ] 7.3 验证 review --output json 输出的 JSON 可被解析
