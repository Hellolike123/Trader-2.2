...
> **最后更新**：2026-08-08（动量双轨展示：位置 / 综合动能 / 买卖盘） | **标杆**：`trader_shared/` 代码 + `formulas.md` + `output-template.md`
...
- **日线威科夫（短线展示双轨）**：
  - **中线看周线、短线看日线短波**（铁律）：中线 `威科夫：` 只读 `wyckoff_midline`；短线 `威科夫：` 只读日线短波侧，**禁止**互相冒充
  - 短线 `威科夫：` ← 日线跑**同一套**种子箱/事件机，**只给人看**；实现 `infer_daily_short_wave` + `format_daily_short_wave_line`（`wyckoff_view`）→ `format_daily_phase_display`
  - **先定侧，再顺着该侧灯**（不是两条链混亮）：
    - 吸筹链：`SC → AR → ST/Spring确认 → LPS → SOS`（池排序 `ACCUM_CHAIN`；skill 灯排同源）
    - 派发链：`BC → ARE → SOW → LPSY → UTAD`（另可亮 UT/PSY；单 ARE 不成派发主波）
    - 成型门槛：吸筹侧关键灯 ∈ {SC,AR,ST,LPS,SOS,Spring}；派发侧 ∈ {BC,LPSY,SOW,UTAD,UT}（ARE 须搭 BC）
    - 簇确认定侧优先：`distribution_confirmed` / `accumulation_confirmed` 直接定主侧（派发确认后近端 SOS 不把侧翻成吸筹）；无簇时两侧都成型才按命中数量择主侧；**主灯**在该侧链内按优先级取一颗（派发波优先 LPSY/SOW/UTAD，避免 BC 永久盖住）
    - 未成型 → 回退箱体/对照 light（`无清晰区间·暂定不出` / `箱体未成形` / 雏形…）；Phase A 失效 → `Phase A 失效｜本波无新SC`（破后可标 SOS/LPS 修复，非正常吸筹链）
  - 文案形态：`短波吸筹|短波派发 · {主灯}{偏多|偏空}[@价] · [箱/雏形?] · 不作买点`；未成型可无「短波*」前缀；**禁止**「日线阶段：」与独立「事件：」；短波已含主灯时**不再叠**反向事件码
  - **不进**中线定论、**不进** fusion、**不进**共振背景岗、**不单独开仓**；灯亮 ≠ 可买
...
**人读顶栏（现行）**：
1. `环境：` 宽基指数 ±% ｜ **主交易板块指数** ±%（须有真实 THS 日线）｜ 强于/弱于/持平板块（个股相对板块超额；**不写个股绝对%**）
2. `概念：` 身份标签（可多选）；**不做概念假指数比较**；概念可映射到真实板块指数（`sector_data`：概念优先，其次 `stock_basic.industry` 别名）
3. `量能：` 量比/换手/调整/位置/ATR14
**不写**正常/偏弱、不写跑赢。环境档仍进 `market_env.level` / fusion `regime`（内部风控），面板不露。
...
| `short_term_momentum` | EXPMA 短期动能（面板顶栏标签为「位置」，字段/词表/门控语义不变） | 走强/修复/震荡/转弱 |
...
中短线**标题挂灯**（`format_track_header_light`）：`🧭 中线｜🔴 防守｜🟡 观望｜🟢 可跟踪`；`⚡ 短线｜🔴 不新开｜🟡 仅观察｜🟢 去看trader`。灯在标题行，不写「操作灯」；绿=资格非可买；块内威科夫/缠论为解释；出手仍听 `decision_view`。

```
分析报告 — {name}（{code}）｜短中线

现价 {price}（{change_pct}）｜MA20 …｜MA250 …
  环境：{宽基短名} ±x% ｜ {主交易板块短名?} ±x% ｜ 强于/弱于/持平板块 …
  概念：{标签1} ｜ {标签2} ｜ {标签3}     ← 身份；无则省略；禁止概念假指数涨跌
  量能：量比… 平/放/缩 ｜ 换手? ｜ 调整N天 ｜ 位置 {momentum} ｜ ATR14 x.xx
  ← 顶栏：环境只放指数/板块对照；位置+ATR 在量能行；板块映射见 §3.4 / sector_data

🧭 中线｜🔴 防守
  威科夫：{wyckoff_midline}     ← 中线状态岗：仅周线；**无**独立「阶段：」/「定论：」行（字段 midline_stage 仍钉周线威科夫）
  量度目标：上 x｜下 y（P&F，非出手）  ← 仅 tr_maturity=L3；无数/未达 L3 则省略
  缠论：{chanlun_midline}       ← 结构副读；周不足可 daily_fallback+「（日线）」；不定阶段
  位置：…
  关键价（中线）
    价格升序阶梯（价在前）· 生命线/黄金/MA / 近压   ← mid_key_prices；关闭态/偏空禁「低吸」；远档>+30%不上梯

⚡ 短线｜🔴 不新开
  缠论：…（日线缠论扳机）
  威科夫：短波吸筹|短波派发 · 主灯[偏多/偏空][@价] · … · 不作买点
  ← 日线短波只对照；先定侧再亮该侧链主灯；禁止「日线阶段：」/独立「事件：」；不进背景岗
  量度目标：上 x｜下 y（P&F，非出手）  ← 仅 L3；与中线分轨；未达则省略
  动能：{偏强|中性|偏弱} · {reason}
  资金：{买盘占优|卖盘占优} · 5日净出/净进{额} · 价资… · 主力x/10·{强势|参与|观望|撤离} · 大单偏买/偏卖
  ← 动能总词读 `fusion.signals_detail.momentum.direction`（1/bullish→偏强，-1/bearish→偏弱，0/neutral→中性）；无 direction 或 reason 含“数据不足”不硬加；`中性 + 动量中性` 只显示 `动能：中性`
  ← 资金行有依据才前置买卖盘（大单优先；其次 `cum_flow_5d_wan` abs>=100 万 或 `cum_flow_10d_wan` abs>=3000 万符号）；不重复量能；主力出口10分制（≥9强势·≥6参与·≥3观望·<3撤离）
  共振 / 新开 / 动作
  关键价（短线）
    止损 / 买点区 / 🌟 现价 / 卖点区  ← 关闭态不铺计划买区（止损+MA5+现价+站稳线）、MA5观察、无仓现价不追
  {买价} 买：亏约… / 赚约…
  {现价} 追：亏约… / 赚约… → 不追?

说明：…
✅ 亮点：…
⚠️ 风险：…                     ← 关闭态优先现价不宜追+止损；勿复读定论「中线偏空」整段
📌 本周只做：…
T0：…                           ← 无仓+关闭态：无底仓不启用；禁日内低吸/高抛
当前池 n/m，回复 1 入池
```

权威全文：`01-功能包-packages/trader/references/output-template.md`。关闭态动词净化见 `docs/plans/trader-panel-declutter-handoff.md`。
...
人读展示：并入顶栏 `量能：` 行（与量比/换手/调整/位置同行），形如 `ATR14 2.58`；无独立 ATR 行。有 `atr_adjust` 时可带复权口径，无有效 `atr14` 则量能行省略 ATR（不必强行 `ATR口径`）。系统固定 **14 日** ATR，勿写成 ATR15。
