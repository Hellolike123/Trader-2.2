...
> **最后更新**：2026-08-07（资金行短扫读 / 主力10分制） | **标杆**：`trader_shared/` 代码 + `formulas.md` + `output-template.md`  
...
现价 {price}（{change_pct}）｜MA20 …｜MA250 …
  ...
  量能：量比… 平/放/缩 ｜ 换手? ｜ 调整N天 ｜ 动能 {momentum} ｜ ATR14 x.xx
  ← 顶栏：环境只放指数/板块对照；动能+ATR 在量能行；板块映射见 §3.4 / sector_data
...
⚡ 短线｜🔴 不新开
  ...
  动能：…
  资金：5日净出/净进{额} · 价资… · 主力x/10·{强势|参与|观望|撤离} · 大单偏买/偏卖
  ← 资金行不重复量能；主力出口10分制（≥9强势·≥6参与·≥3观望·<3撤离）
  共振 / 新开 / 动作
  关键价（短线）
...
人读展示：并入顶栏 `量能：` 行（与量比/换手/调整/动能同行），形如 `ATR14 2.58`；无独立 ATR 行。有 `atr_adjust` 时可带复权口径，无有效 `atr14` 则量能行省略 ATR（不必强行 `ATR口径`）。系统固定 **14 日** ATR，勿写成 ATR15。
