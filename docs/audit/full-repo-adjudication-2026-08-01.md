...
| **C-F04** | cards→classic silent fallback | **DOWNGRADE → DEFER** | `fusion_core` 默认 cards 失败时 `_logger.warning` 后 classic。**BUSINESS §2.7 明文允许**「失败打 warning 再 classic」。非 silent；非违约 |
...
| **D2 / C-F07** | `mid_bullish_downgrade` 松开空仓 | ✅ **已删**（PR #18 / `test_fusion_instrument_caps`）：`attach_short_midline` 不再改写 discipline.action |
| **D3 / C-F08** | pool `watch` 仍按 `status==执行` 分流 | ✅ 主序改为 `lane==ready`（PR #20）；无 lane 时仍回退 `status==执行` 作兼容 |
| **D4 / C-F09** | strategy `entry_reason` 文案 | ✅ 已改为 `可扳机`（闸门仍 `executable=True`；避免与 T0「可执行」禁令混淆） |
| **D5 / C-F10** | `FUSION_LOG_ONLY` import-time 常量 | ✅ 已改为 `_fusion_log_only()` 运行时读 env（无需 reload） |
...
Phase E (defer queue)           D2–D5 已收口；余 D1/D6/D8
```
...
**DEFER**: classic 回退（法源允许）· 其它 P2  
（已落地：D2 mid_bullish 删除；D3 watch 优先 `lane==ready`；D4 `entry_reason=可扳机`；D5 FUSION_LOG_ONLY 运行时读）  
