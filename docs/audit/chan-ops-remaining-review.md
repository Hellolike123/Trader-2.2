...
> 日期：2026-07-10  
> 验收唯一规格：`docs/chan-ops-remaining-backlog-plan.md`（V1–V10）  
> 参考：`docs/chan-ops-playbook.md` · `docs/chan-discipline-b-plan.md`  
...
R1–R10 已在 `chan_discipline` / `run_analysis` / `report_core` / 单测 / `output-template` 链路落地。  
原则核对：只收紧不放宽；`chan_core` 无禁止开仓；买点阶梯 / 盘整 / low_zone / 分闸 / 位置 / 同级 / 破位 / 周框均有实现与测试。  
...
| V6 | 报告含位置：中枢… | **PASS** | `compute_pivot_position` → `中枢内\|中枢上(回踩中)\|中枢下(反抽中)\|中枢外\|未知`（L104–143）。`run_analysis` 写 `pivot_position_weekly/daily`（L1588–1589）。`report_core` 🧭/⚡ 各一行 `位置：…`（L179–186, L232–237）。`output-template.md` 样例已补。`TestR6PivotPosition` |
| V7 | 缠论文案含（同级）当有买卖点/背驰 | **PASS** | `needs_same_level_tag` / `append_same_level_tag`（L733–769）。`report_core` 中线缠论 + 短线缠论均追加（L164–177, L217–228）。`TestR7SameLevelTag`；template 样例 `顶背驰 · 看跌（同级）` |
...
| N4 | R7 无 `report_core` 渲染级单测（逻辑 + template 已齐） | 可选快照测 |
