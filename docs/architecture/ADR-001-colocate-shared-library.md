...
- **Status**: Accepted（已落地：`pipeline` / `signal_tracker` / `market_env` / `calibrator` / `self_calibration` 均在 `trader_shared/`；向上依赖已消除）
- **Branch**: `refactor/trader-architecture`（基于 `0b48fdc`，ADR-004 已落地；后续 main 已合入）
