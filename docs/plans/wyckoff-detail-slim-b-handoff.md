...
2. 失败态日线：仅旧破写 `PhaseAFail`；若 failed 后仍亮 SOS/LPS，顶栏写 `旧破·其后SOS`（或 `其后SOS+LPS`），禁止只丢一个 `PhaseAFail` 让人以为「现在很差」。
...
```text
SC后反弹偏多，雏形 37.80～43.85（待 ST）｜未达 L3
旧筑底已破（SC 41.02）｜无箱｜旧吸筹链停止推进（≠现在一定很差）
旧筑底已破（SC 27.71）｜其后SOS（强势信号）（破后强势，不是原吸筹链复活）｜无箱
```
...
4. **换幕两幕（突然拉升类，用户 2026-08-02 确认方案 1）**：  
   - 触发：`phase_a=failed` 且破后仍亮 SOS/LPS。  
   - 顶栏：`日：换幕（旧吸筹结束→破后强势）`。  
   - `⚡ 短线 · 当前幕`：只讲破后强势 + 只列 SOS/LPS 灯 + 下一盯回踩站稳。  
   - `📎 上一幕（已结束）`：旧 SC→破位作废；SC 灯标「历史，不参与当前推进」。  
   - 禁止 `SC→SOS（Phase A 已失效）` 拧句。  
   - 仅 failed、无破后强势：不换幕，仍「待新寻底」。  
5. **周线派发侧**：亮 ARE/BC/SOW 等时，下一盯走派发逻辑；ARE 无 BC → `派发未确认（缺 BC），中线观望`，禁止接吸筹 `○ SC 下一盯`。日线换幕与周线偏冷可并存，中线可注「与日线换幕可并存」。
...
| S-B18 | 换幕两幕（突然拉升类） | failed+SOS：顶栏 `日：换幕（旧吸筹结束→破后强势）`；`⚡ 短线 · 当前幕` 只列破后强势灯；`📎 上一幕` 收旧 SC；无 `SC→SOS（已失效）` |
| S-B19 | 周线派发侧勿接吸筹下一盯 | 周线 ARE 无 BC：下一盯为派发未确认/观望，不得 `○ SC 下一盯` |
