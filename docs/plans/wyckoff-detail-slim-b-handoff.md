...
`🔮 推演` **必须保留**（用户确认：B 卡不能缺推演故事）。相对旧 `--full`「故事链」：每段一行，无重复入池/说明长文；失败态遵守 failed-chain-copy（禁「还差AR / 链可推进」）。
...
🔮 推演
现在：日线 SC（Phase A 已失效）｜周线 SC→AR，待ST
若变好：日线重新寻底并出现新 SC（卖力高潮）；周线出 ST（二次测试）确认雏形
若变坏：日线继续破位走弱则旧链彻底作废；周线失守 37.80 一带则雏形作废
⭐ 盯：日线等新 SC；周线看能否出 ST（二次测试）确认结构
本卡不下单；出手/分道看 trader
```
...
1. 固定四行语义：`现在：` / `若变好：` / `若变坏：` / `⭐ 盯：`，外加一行 `本卡不下单；出手/分道看 trader`。
2. 失败态日线「若变好」写重新寻底 / 新 SC，不得写「若出现 AR…链可推进」或「还差」。
3. 「若变坏」可引用周线批准下沿（L1+），L0 不得拿分位沿冒充。
4. 不恢复旧 `--full` 标题 `🔮 故事链（以日线推进；周线作背景）`，不用空行小标题拆成五段长文。
5. 不写底部重复入池长文。
...
| S-B13 | `⭐ 盯` 固定短提示 | 默认 B 推演块含 `⭐ 盯：…` 与 `本卡不下单；出手/分道看 trader`，且不再有说明长文 |
