...
1. 1 到 2 行观察句。
2. 失败态日线写重新寻底 / 新 SC；周线可写 ST / LPS 等下一确认。
3. 固定追加短提示：`本卡不下单；出手/分道看 trader`。
4. 不写底部重复入池长文，不写“说明：买卖看 trader...”长段。
...
| S-B4 | 默认 B 砍掉常驻长块 | 默认输出不含 `📊 现况`、`🔮 故事链`、`💬 综述`，且无底部入池长说明 |
...
| S-B13 | `⭐ 盯` 固定短提示 | 默认 B 在 `⭐ 盯` 下含 `本卡不下单；出手/分道看 trader`，且不再有说明长文 |
