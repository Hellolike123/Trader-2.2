...
4. **旧破 vs 破后强势必须拆开**（顶点软件类）：禁止 `SC→SOS（Phase A 已失效）`；有 SOS/LPS 时写「其后…（破后强势，不是原吸筹链复活）」；下一盯改为回踩站稳，不假装还在「重新寻底」。
...
| S-B18 | 旧破+破后强势拆开 | failed+SOS fixture：顶栏/短线/推演拆开「旧破」与「其后SOS」；无 `SC→SOS（Phase A 已失效）`；下一盯含回踩站稳而非健康还差AR |
