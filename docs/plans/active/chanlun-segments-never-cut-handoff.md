...
| 场景 | 期望 |
|------|------|
| 9 笔变体：向上段被向下笔 low 跌破段起点 low（第 6 笔 15→9.5 破 10），破坏后剩余 ≥3 笔 | ≥2 段：先 up + down |
| 9 笔对称：向下段被向上笔 high 突破段起点 high，破坏后剩余 ≥3 笔 | ≥2 段：先 down + up |
| 破坏笔为末笔或破坏后剩余笔 < min_strokes（如 §1.2 的 6 笔样例） | 1 段（§3.6 护栏防过切，不切） |
...
> 注（2026-08-08 审计对齐）：§1.2 的 6 笔复现样例中破坏笔为末笔，破坏后剩余笔数不足 `min_strokes`，按 §3.6 护栏不切段；切割行为以 9 笔变体（`test_chan_segments_bug_r.py`）为准。若产品要求末笔破坏也切，须先改 §3.6 护栏定义，再改实现与测试。
