...
- **W-DIFF-7 深刺穿收回与 ST**：破位仅当 `low < sc_low×(1−MAX_PIERCE)` **且** `close < sc_low`；深刺穿但 `close ≥ sc_low` 收回 → 不算破位；未 failed 且满足既有 ST 条件时**允许**认 ST（不因「曾刺穿过 floor」单独否决）。法源：`w-diff7-st-pierce-decision-handoff.md` + structure-anchor §3.1；**不改** ST 检测阈值/公式。
